...
checkEmptyStdout \
  "git grep -l -i -I -P '[^a-f0-9][a-f0-9]{32}[^a-f0-9]' | fgrep -e external/uwp_ssl/include/openssl/bn.h -e tests/scripts/test-server.js -e docs/articles/usp-translation.md -e public_samples/quickstart/unity/helloworld/Assets/SpeechSDK/Scenes/HelloWorld.unity -e public_samples/quickstart/unity/helloworld/ProjectSettings/GraphicsSettings.asset -v" \
...
checkEmptyStdout \
  "git grep -l -i -I carbon public_samples source/public source/bindings/csharp source/bindings/objective-c ThirdPartyNotices.md REDIST.txt license.md | grep -v -e CMakeLists\.txt$ -e carbon_[a-z]*\.i$" \
