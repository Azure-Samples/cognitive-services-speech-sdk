...
checkEmptyStdout \
  "git grep -I -l \$'\t' $gitTree | cut -d: -f2- | grep -v ^external/ | grep -v \.sln$ | grep -v /Makefile$ | grep -v project\.pbxproj | grep -v \.plist | grep -v ci/licensefiles/ | grep -v -e sampledata/.*\.txt -e public_samples/sampledata/.*\.txt" \
...
checkEmptyStdout \
  "git grep -I -l $'\xEF\xBB\xBF' $gitTree | cut -d: -f2 | grep -v -e sampledata/.*\.txt -e public_samples/sampledata/.*\.txt" \
...
checkEmptyStdout \
  ...
  -e 'public_samples.*(asset|mat|table)$' -e 'docs.*(usp-translation.md)$' -e 'public_samples.*(HelloWorld.unity)$' -e 'public_samples.*(IntentRecognition.unity)$' -e 'public_samples.*(SpeechRecognition.unity)$' -e 'public_samples.*(SampleDialogServiceConnector.unity)$' -e 'public_samples.*(KwsSample.unity)$'\
  -e ci/docker/snippets/stage_dotnet_binary_centos8_x64.txt \
  -v | cut -d: -f2-" \
