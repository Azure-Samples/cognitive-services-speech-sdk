...
checkEmptyStdout \
  "git grep -I -l \$'\t' $gitTree | cut -d: -f2- | grep -v -e ^external/ -e \.sln$ -e /Makefile$ -e project\.pbxproj -e Info\.plist -e sampledata/.*/pronunciation\.txt" \
...
checkEmptyStdout \
  "git grep -I -l $'\xEF\xBB\xBF' $gitTree | cut -d: -f2 | grep -v sampledata/.*/pronunciation\.txt" \
