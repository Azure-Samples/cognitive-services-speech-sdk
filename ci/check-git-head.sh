...
checkEmptyStdout \
  ...
  -e 'public_samples.*(asset|mat)$' -e 'ci/latency.*(run-latency-test.py)$' -e 'docs.*(usp-translation.md)$' -e 'public_samples.*(HelloWorld.unity)$' -e 'public_samples.*(IntentRecognition.unity)$' -e 'public_samples.*(SpeechRecognition.unity)$' -e 'public_samples.*(SampleBotConnector.unity)$' \
  -v | cut -d: -f2-" \
