...
checkEmptyStdout \
  "git grep -l -i -I -P '[^a-f0-9][a-f0-9]{32}[^a-f0-9]' $gitTree | egrep -v -e 'public_samples.*(asset|mat)$' -e 'ci/latency.*(run-latency-test.py)$' -e 'docs.*(usp-translation.md)$' -e 'public_samples.*(HelloWorld.unity)$' -e 'public_samples.*(IntentRecognition.unity)$' -e 'public_samples.*(SpeechRecognition.unity)$' | cut -d: -f2-" \
