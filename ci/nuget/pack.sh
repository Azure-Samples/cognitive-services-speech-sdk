...
# Check that Android drop directory exists and turn into Windows path.
ANDROID_DROP_DIR="$(cygpath --unix --absolute "$DROP_DIR/Android")"
[[ -e $ANDROID_DROP_DIR ]]
ANDROID_DROP_DIR="$(cygpath --windows --absolute "$ANDROID_DROP_DIR")"

# Check that iOS drop directory exists and turn into Windows path.
iOS_DROP_DIR="$(cygpath --unix --absolute "$DROP_DIR/IOS")"
[[ -e $iOS_DROP_DIR ]]
iOS_DROP_DIR="$(cygpath --windows --absolute "$iOS_DROP_DIR")"

OUTPUT_DIR="$(cygpath --windows --absolute "$OUTPUT_DIR")"

"$NUGETEXETOOLPATH" pack "$NUSPEC_FILE_PATH" \
  -Properties "RedistDir=$REDIST_DIR;WindowsDropDir=$WINDOWS_DROP_DIR;WindowsOSDropDir=$WINDOWS_OS_DROP_DIR;WindowsUwpDropDir=$WINDOWS_UWP_DROP_DIR;LinuxDropDir=$LINUX_DROP_DIR;OSXDropDir=$OSX_DROP_DIR;AndroidDropDir=$ANDROID_DROP_DIR;iOSDropDir=$iOS_DROP_DIR;Version=$VERSION" \
