...
function combine_dsym_bundles() {
    local USAGE="Usage: $0 OUTPUT_DIR SIMULATOR64_LIB DEVICE_LIB"

    local output_dir=${1?$USAGE}
    local simulator64_lib=${2?$USAGE}
    local device_lib=${3?$USAGE}

    set -u -e -o pipefail -x
    for lib in $simulator64_lib $device_lib; do
      dsymutil ${lib} -o ${lib}.dSYM
    done

    libname=$(basename ${simulator64_lib})
    # copy files from one of the frameworks
    cp -R "${simulator64_lib}.dSYM" "${output_dir}/${libname}.dSYM" && \
