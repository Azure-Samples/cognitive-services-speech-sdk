...
public class SpeechRecognitionSamples {
    ...
    private static String getChatCompletion(String oaiResourceName, String oaiDeploymentName, String oaiApiVersion, String oaiApiKey, String sendText) throws IOException, InterruptedException {
        ...
        userMessageBuilder.add("content", String.format("Example1: this essay: \"%s\" has vocabulary and grammar scores of 80 and 80, respectively. " +
                ...
                sampleSentence1, sampleSentence2, sampleSentence3, sendText, topic));
        JsonObject userMessage = userMessageBuilder.build();
        ...
        requestBodyBuilder.add("messages", messages);
        requestBodyBuilder.add("temperature", 0);
        requestBodyBuilder.add("top_p", 1);
        ...
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .header("api-key", oaiApiKey)
                .header("Content-Type", "application/json")
                .POST(HttpRequest.BodyPublishers.ofString(jsonData)) // Send the data as a POST request
                .build();
        ...
        // Parse the response content using Jakarta JSON
        JsonObject jsonResponse = Json.createReader(new java.io.StringReader(responseContent)).readObject();
    ...
    // See more information at https://aka.ms/csspeech/pa
    public static void pronunciationAssessmentWithContentAssessment() throws ExecutionException, InterruptedException, URISyntaxException {
        // subscription key and endpoint URL. Replace with your own subscription key
        // and endpoint URL.
        SpeechConfig config = SpeechConfig.fromEndpoint(new URI("YourEndpointUrl"), "YourSubscriptionKey");

        String oaiResourceName = "YourAoaiResourceName";
        String oaiDeploymentName = "YourAoaiDeploymentName";
        String oaiApiVersion = "YourAoaiApiVersion";
        String oaiApiKey = "YourAoaiApiKey";
        ...
        SpeechRecognizer recognizer = new SpeechRecognizer(config, lang, audioInput);
        {
            ...
            System.out.println("Content assessment for: " + recognizedText);
            try {
                String content = getChatCompletion(oaiResourceName, oaiDeploymentName, oaiApiVersion, oaiApiKey, recognizedText);

                // Parsing the JSON string
                JsonReader reader = Json.createReader(new StringReader(content));
                JsonObject jsonObject = reader.readObject();

                // Extracting individual scores from the JSON object
                double vocabularyScore = jsonObject.getJsonNumber("vocabulary").doubleValue();
                double grammarScore = jsonObject.getJsonNumber("grammar").doubleValue();
                double topicScore = jsonObject.getJsonNumber("topic").doubleValue();
