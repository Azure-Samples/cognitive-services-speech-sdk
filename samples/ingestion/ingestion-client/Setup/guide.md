...
Get your audio files automatically transcribed without any development effort! The Ingestion Client will monitor your dedicated Azure Storage container so that new audio files are transcribed automatically as soon as they land.
...
We created an ingestion layer (a transcription client) that will help you set-up a full blown, scalable and secure transcription pipeline. Using ARM template deployment, all the resources necessary to seamlessly process your audio files are wired-up and set in motion.

# Architecture

This is a smart client that implements best practices and is optimized against the capabilities of the Azure Speech infrastructure. It utilizes Azure resources, to orchestrate transcription requests to Azure Speech Service, from audio files landing in your dedicated storage containers. Do you need more than transcription? Do you need to applly Sentiment to your transcript? Downstream analytics are possible too with Text Analytics. 
...
2. Retry logic optimization to handle smooth scaling up and transient HTTP 429 errors
3. Running Azure Functions economically, ensuring minimal execution costs
4. Round Robin around selected regions in order to distribute load across available regions (per customer request)
...
An [Azure Account](https://azure.microsoft.com/free/) as well as an [Azure Speech Services key](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesSpeechServices) is needed to run the Ingestion Client.
...
4. Multiple Audio Formats are supported
5. You will need to deploy the [Batch ARM Template](ArmTemplateBatch.json) from the repository for this operating mode.
...
In Real time mode, each Azure Speech resource is allocated by default 50 concurrent connections, which indicates the maximum number of parallel audio transcription streams. Customers do have a choice to raise a request to have that number increased through Azure Speech customer channels. To avoid throttling however the rythm/tempo of uploading audio files to Azure storage (therefore triggering real time transcriptions) should be controlled. If it is not contolled it will result in throttling.
...
By default, the ARM template uses the newest version of the Ingestion Client which can be found in this repository. If a custom version should be used, the paths to the binaries inside the deployment template must be edited to point to a custom published version (by default, our binaries are: 
