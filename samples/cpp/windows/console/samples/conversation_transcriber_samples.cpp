...
// Transcribing conversation using a microphone using audio processing options to
// capture 7 + 1 reference channel audio (requires compatible hardware to be present).
void ConversationTranscriptionWithMicrophoneUsingAudioProcessingOptions()
{
    ...
    // Creates a participant. Please note that you should create a voice signature matching an actual conversation participant
    // and use that instead of the sample provided here.
    auto katie = Participant::From("katie@example.com", "en-us", voiceSignatureKatie);

    // creates another participant. Please note that you should create a voice signature matching an actual conversation participant
    // and use that instead of the sample provided here.
...
// Transcribing conversation using a pull audio stream with 7 + reference channel audio
void ConversationTranscriptionWithPullAudioStreamUsingMultichannelAudio()
{
    ...
    transcriber->Canceled.Connect([&recognitionEnd](const ConversationTranscriptionCanceledEventArgs& e)
    {
        switch (e.Reason)
        {
        case CancellationReason::EndOfStream:
...
void ConversationTranscriptionWithMultichannelAudioFile()
{
...
void ConversationTranscriptionWithPushAudioStreamUsingMultichannelAudio()
{
    // Creates an instance of a speech config with your subscription key and region.
    // Replace with your own subscription key and service region (e.g., "eastasia").
    // Conversation Transcription is currently available in eastasia and centralus region.
