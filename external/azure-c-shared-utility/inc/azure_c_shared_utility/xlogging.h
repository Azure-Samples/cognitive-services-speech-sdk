...
#if defined _MSC_VER
#define LogError(FORMAT, ...) do{ time_t t = time(NULL); struct tm ltm; localtime_s(&ltm, &t); LOG(LOG_ERROR, LOG_LINE, "Error: Time:" ISOPRINTFMT " File:%s Func:%s Line:%d " FORMAT, ISOPRINTARGS(&ltm), __FILE__, __FUNCDNAME__, __LINE__, __VA_ARGS__); }while(0)
#else
#define LogError(FORMAT, ...) do{ time_t t = time(NULL); struct tm *ptm = localtime(&t); LOG(LOG_ERROR, LOG_LINE, "Error: Time:" ISOPRINTFMT " File:%s Func:%s Line:%d " FORMAT, ISOPRINTARGS(ptm), __FILE__, __func__, __LINE__, ##__VA_ARGS__); }while(0)
#endif
