...
def test_audio_transcription_for_codecs(http_codecs_test_data_tc):
    ...
    with allure.step(f"GET scrid {scrid} response from HTTP Listener"):
        ...
        response_body_element_key = ['scrid', 'externalRequestID', 'reasonStatus', 'convertedText']
        ...
        validate(get_config_value('httptest', 'caller_id'), xml_response_dict['externalRequestID'],
                 'Validate Response Body externalRequestID')
        validate(test_data_tc['verify_audio_converted_status'], xml_response_dict['reasonStatus'],
                 'Validate Response Body reasonStatus')
