...
TEST_CASE("Audio Basics", "[api][cxx][audio]")
{
    ...
    SPXTEST_REQUIRE(exists(filePath));
    ...
    SPXTEST_SECTION("Audio Config Properties")
    {
        auto verifyAudioConfigInRecognizerProperties = [](std::shared_ptr< SpeechRecognizer> recognizer, std::string deviceNameExpected = "",  std::string channelsExpected = "1")
        {
            ...
            SPXTEST_REQUIRE(deviceNameExpected == deviceNameInRecognizer);
            ...
            SPXTEST_REQUIRE(channelsExpected == channelsInRecognizer);
            ...
            SPXTEST_REQUIRE(sampleRateExpected == sampleRateInRecognizer);
            ...
            SPXTEST_REQUIRE(bitsPerSampleExpected == bitsPerSampleInRecognizer);
        };

        SPXTEST_WHEN("verify audio configuration from audio file")
        {
        ...
        SPXTEST_WHEN("verify audio configuration with device name and channel count")
        {
            ...
            SPXTEST_REQUIRE(channelsExpected == channelsInAudioConfig);
            ...
            SPXTEST_REQUIRE(deviceNameExpected == deviceNameInAudioConfig);
    ...
    SPXTEST_SECTION("pull stream works")
    {
    ...
    SPXTEST_SECTION("push stream works")
    {
