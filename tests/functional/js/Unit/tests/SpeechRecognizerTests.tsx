...
//??     assertNotNull(r.getRecoImpl());
    ...
    const res: sdk.SpeechRecognitionResult = r.recognizeAsync().Result().Result;
...
test("testGetOutputFormatDetailed", ()=> {
    ...
    const language:string = "de-DE";
    const r: sdk.SpeechRecognizer = s.createSpeechRecognizerWithFileInputAndLanguageAndOutput(Settings.WaveFile, language, sdk.OutputFormat.Detailed);
    expect(r).not.toBeUndefined();
...
test("testRecognizeAsync1", done => {
    ...
    const recPromise: sdk.Promise<sdk.SpeechRecognitionResult> = r.recognizeAsync()
...
test("testRecognizeAsync2", done => {
    ...
    r.recognizeAsync().On((res: sdk.SpeechRecognitionResult) => {
        ...
        if ("IntermediateResultReceived" in eventsMap)
            expect(eventsMap["IntermediateResultReceived"]).toBeGreaterThan(eventsMap[sdk.RecognitionEventType.SpeechStartDetectedEvent.toString()]);
...
test("testStartContinuousRecognitionAsync", done => {
    ...
    const future: sdk.Promise<void> = r.startContinuousRecognitionAsync();
    expect(future).not.toBeUndefined();
...
//@Test
test("testStopContinuousRecognitionAsync", () => {
    const s: sdk.SpeechFactory = sdk.SpeechFactory.fromSubscription(Settings.SpeechSubscriptionKey, Settings.SpeechRegion);
    expect(s).not.toBeUndefined();
    ...
    let future: sdk.Promise<void> = r.startContinuousRecognitionAsync();
    expect(future).not.toBeUndefined();

    expect(future.Result().IsCompleted).toEqual(true);
    expect(future.Result().IsError).toEqual(false);
    ...
    future.On(() => {
        ...
        WaitForCondition(() => {
            ...
            () => {
                ...
                // TODO: the javascript implementation currently returns an additional empty
                //       response when calling the API with a file, thus tolerate a second result.
                expect(rEvents.length).toBeGreaterThanOrEqual(1);
                expect(rEvents.length).toBeLessThanOrEqual(2);

                const stopFuture: sdk.Promise<void> = r.stopContinuousRecognitionAsync();
                expect(future).not.toBeUndefined();

                WaitForCondition(() => { return future.Result().IsCompleted; },
                    () => {
                        expect(future.Result().IsCompleted).toEqual(true);
                        r.close();
                        s.close();
...
function WaitForCondition(condition: () => boolean, after: () => void) {
    if (condition() === true) {
        after();
    }
    else {
