...
@pytest.mark.skip('Skip until skyman supports DGI')
...
def test_intent_recognition_phraselist(from_file_intent_reco_with_callbacks, intent_input):
    ...
    # setup PhraseListGrammar
    pg = msspeech.PhraseListGrammar.from_recognizer(intent_recognizer)
...
@pytest.mark.parametrize('intent_input,', ['beach_nohelp_truman'], indirect=True)
def test_intent_recognition_phraselist_clear(from_file_intent_reco_with_callbacks, intent_input):
    intent_recognizer, _ = from_file_intent_reco_with_callbacks()
