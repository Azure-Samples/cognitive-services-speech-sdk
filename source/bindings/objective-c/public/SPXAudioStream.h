...
/**
 * Base class representing audio input stream used for custom audio input configurations.
...
/**
  ...
  * @param readHandler handler which will be called in order to read data from the audio input stream.  If no data is currently available in the stream, the readHandler should wait until data is available. It returns the number of bytes that have been read. It returns 0 when stream is closed.
