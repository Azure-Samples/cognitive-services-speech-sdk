...
{
    try {
        auto configImpl = SpeechImpl::SpeechConfig::FromHost([hostUri toSpxString], [subscriptionKey toSpxString]);
        if (configImpl == nullptr)
            return nil;
        return [self initWithImpl:configImpl];
    }
...
{
    try {
        auto configImpl = SpeechImpl::SpeechConfig::FromHost([hostUri toSpxString]);
        if (configImpl == nullptr)
            return nil;
        return [self initWithImpl:configImpl];
    }
