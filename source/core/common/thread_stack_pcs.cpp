        ...
        unsigned skip, void *startfp)
{
    ...
    uintptr_t stacktop = reinterpret_cast<uintptr_t>(pthread_get_stackaddr_np(self));
    uintptr_t stackbot =  stacktop - reinterpret_cast<uintptr_t>(pthread_get_stacksize_np(self));
    ...
    *nb = 0;
    ...
    next = pthread_stack_frame_decode_np((uintptr_t)frame, NULL);
    ...
    if(!INSTACK(frame) || !ISALIGNED(frame))
        return;
    while (startfp || skip--) {
        if (startfp && (size_t)startfp < next) break;
    ...
    while (max--) {
        uintptr_t retaddr;
        next = pthread_stack_frame_decode_np((uintptr_t)frame, &retaddr);
        buffer[*nb] = retaddr;
        (*nb)++;
        if(!INSTACK(next) || !ISALIGNED(next) || next <= frame)
            return;
