...
namespace Debug
{
    extern void _thread_stack_pcs(vm_address_t *buffer, unsigned max, unsigned *nb, unsigned skip, void *startfp);

    int carbon_backtrace(void** buffer, int size) {
        unsigned int num_frames;
        Debug::_thread_stack_pcs((vm_address_t*)buffer, size, &num_frames, 1, NULL);
        while (num_frames >= 1 && buffer[num_frames - 1] == NULL) num_frames -= 1;
        return num_frames;
    }
    int carbon_backtrace_snprintf(char* buf, size_t size, int frame, const void* addr, const Dl_info* info) {
    ...
    static size_t carbon_symbol_length(Dl_info *info)
    {
        if (info->dli_sname) {
            if (strcmp(info->dli_sname, "<redacted>") == 0) {
    ...
    char** carbon_backtrace_symbols(void* const* buffer, int size) {
        ...
        // The block will contain size number of pointers to thevar info[0]
        ...
        for (i = 0; i < size; ++i) {
            ...
            total_bytes += carbon_symbol_length(&info[i]);
        }
        ...
        for (i = 0; i < size; ++i) {
            int chk = Debug::carbon_backtrace_snprintf((char*)strs, end - (intptr_t)strs, i, buffer[i], &info[i]);
            if (chk < 0) {
                goto error;
            }
